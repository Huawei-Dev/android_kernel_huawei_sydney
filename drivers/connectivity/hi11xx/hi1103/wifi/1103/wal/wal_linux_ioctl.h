...
#define WAL_PHY_DEBUG_TEST_WORD_CNT     5           /* trailer???????? */
...
#define WAL_IOCTL_PRIV_SET_MODE     (OAL_SIOCIWFIRSTPRIV + 13)      /* ???????? ???????? ???? ???? */
...
#define WAL_IOCTL_PRIV_SET_MGMT_FRAME_FILTERS        (OAL_SIOCIWFIRSTPRIV + 28)      /* ?????????????? */
#ifdef _PRE_WLAN_FEATURE_HILINK_HERA_PRODUCT_DEBUG
#define WAL_IOCTL_PRIV_SET_VENDOR_REQ                (OAL_SIOCIWFIRSTPRIV + 30)     /* HERA????????????IOCTL?????? */
#endif
...
typedef enum
{
    WAL_DSCR_PARAM_PA_GAIN_LEVEL = 0,      /* pa???????? */
...
{
    oal_int8                           *pc_name;        /* ?????????????? */
    wlan_cfgid_enum_uint16              en_tlv_cfg_id;     /* ???????????????????? */
