                ...
                mac_cfg_add_vap_param_stru *pst_param)
{
    ...
    switch(pst_param->en_vap_mode)
    {
        ...
        pst_hmac_vap->bit_rx_ampduplusamsdu_active = OAL_TRUE;  //51??AP??????vap??????????????????AMPDU+AMSDU????????
    ...
    pst_hmac_vap->en_restrict_band = WLAN_BAND_BUTT;    // ??????????
#endif
#if defined(_PRE_WLAN_FEATURE_DBAC) && defined(_PRE_WLAN_FEATRUE_DBAC_DOUBLE_AP_MODE)
    pst_hmac_vap->en_omit_acs_chan = OAL_FALSE;         // ??????????
#endif
